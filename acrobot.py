...
class AcrobotEnv(core.Env):
    ...
    **REFERENCE:**
    ...
    def step(self, a):
        ...
        ns = rk4(self._dsdt, s_augmented, [0, self.dt], self.rk)    # 引数self.rkを追加
    ...
    def set_rk(self, x):    # ルンゲクッタの近似次数を動的に変更するための関数
        self.rk = x
...
def rk4(derivs, y0, t, rk, *args, **kwargs):    # 引数rkを追加
    """
    ...
    *kwargs*
    ...
    for i in np.arange(len(t) - 1):
        ...
        if rk == 1:
            ...
            k1 = np.asarray(derivs(y0, thist, *args, **kwargs))
            k2 = np.asarray(derivs(y0 + dt2 * k1, thist + dt2, *args, **kwargs))
            k3 = np.asarray(derivs(y0 - dt * k1 + 2 * dt * k2, thist + dt, *args, **kwargs))
            yout[i + 1] = y0 + dt / 6.0 * (k1 + 4 * k2 + k3)
        else:
            k1 = np.asarray(derivs(y0, thist, *args, **kwargs))
            k2 = np.asarray(derivs(y0 + dt2 * k1, thist + dt2, *args, **kwargs))
