...
for learn_num in range(1, LEARN_NUM + 1):
    ...
    reward_reach_ave = []
    reward_reach_time = []  # テスト時の報酬が-400，-300，-200，-100になった時刻を格納するリスト

    EPSILON = 1.0  # ε-greedy法
    RK_THRESHOLD = 0
    rk = 1  # 現在の近似次数

    # テスト時の報酬が-400，-300，-200，-100になったかどうかを表すフラグ
    flag_400 = True
    flag_300 = True
    flag_200 = True
    flag_100 = True
    ...
    while total_step < STEP_NUM and rk < 4 and flag_100:

        # trainモード
        for train_epoch in range(RK_TRAIN_NUM):
            # 10エピソードごとに5エピソードテスト
            if (train_epoch + 1) % EPISODE_TRAIN == 0:
                stop = time.time()
                env.set_rk(4)
                total_reward = 0  # 累積報酬5エピソード分の総和
                for episode in range(EPISODE_TEST):
                ...
                ave = total_reward / EPISODE_TEST  # 5エピソードの平均を計算
                # 報酬が各値を超えていたらフラグをFalseにし，その時の時刻をリストに格納
                if ave >= -400 and flag_400:
                    flag_400 = False
                    reward_reach_ave.append(ave)
                    reward_reach_time.append(stop - start)
                if ave >= -300 and flag_300:
                    flag_300 = False
                    reward_reach_ave.append(ave)
                    reward_reach_time.append(stop - start)
                if ave >= -200 and flag_200:
                    flag_200 = False
                    reward_reach_ave.append(ave)
                    reward_reach_time.append(stop - start)
                if ave >= -100 and flag_100:
                    flag_100 = False
                    reward_reach_ave.append(ave)
                    reward_reach_time.append(stop - start)

                env.set_rk(rk)
                start = start + (time.time() - stop)  # テストにかかった時間は含めない
    ...
    # 結果の保存
    f = open('reward_reach_ave_c_' + str(learn_num) + '.txt', 'wb')
    pickle.dump(reward_reach_ave, f)
    f = open('reward_reach_time_c_' + str(learn_num) + '.txt', 'wb')
    pickle.dump(reward_reach_time, f)
...
for learn_num in range(1, LEARN_NUM + 1):
    ...
    # 学習開始
    start = time.time()
    while total_step < STEP_NUM and flag_100:
        # 10エピソードtrain
    ...
    # 結果の保存
    f = open('reward_reach_ave_4_' + str(learn_num) + '.txt', 'wb')
    pickle.dump(reward_reach_ave, f)
    f = open('reward_reach_time_4_' + str(learn_num) + '.txt', 'wb')
    pickle.dump(reward_reach_time, f)
...
for learn_num in range(1, LEARN_NUM + 1):
    ...
    # 結果の保存
    f = open('reward_reach_ave_1_' + str(learn_num) + '.txt', 'wb')
    pickle.dump(reward_reach_ave, f)
    f = open('reward_reach_time_1_' + str(learn_num) + '.txt', 'wb')
    pickle.dump(reward_reach_time, f)
